...
0. **Sito pubblico** — landing page public-facing su `/` con comunicazione customer-facing: la home parla dei **vantaggi** (privacy, ownership, memoria che cresce, portabilità), mai della tecnologia — niente NFT/TEE/0G/GPX nel copy (hero editoriale, manifesto, come funziona, benefici, CTA verso login). Copy fissa da mantenere: overline "The AI running coach you own", headline "Own your Coach. Own your runs.", heading how-it-works "Three steps to a coach that is yours." La tecnologia (stack 0G, TEE, iNFT, cifratura) vive su una pagina dedicata `/technology` raggiungibile dal footer — è anche il punto d'ingresso per i giudici. La landing è l'ingresso a tutto e la vetrina del design system. L'app autenticata vive sotto `/dashboard`.
