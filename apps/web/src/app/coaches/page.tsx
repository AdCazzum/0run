...
export const metadata: Metadata = {
  ...
  description:
    "Every 0run coach agent that exists on-chain, enumerated from the ERC-7857 agent registry and identified live through ENS.",
};
...
export default async function CoachesDirectoryPage() {
  ...
  try {
    entries = await getCoachDirectory();
  } catch (e) {
  ...
  return (
    <main className="relative mx-auto max-w-[1600px] px-8 py-20 md:px-16 md:py-32">
      ...
      <p className="mt-8 max-w-xl font-sans text-lg leading-relaxed text-navy">
        Enumerated straight from the agent registry on 0G Galileo, then identified live through ENS —
        never a database claiming to be the list. What&apos;s below is what exists right now.
      </p>

      {loadError && (
        <p className="mt-12 max-w-xl border-t border-navy/15 pt-8 font-sans text-sm leading-relaxed text-ocean">
          Could not read the on-chain registry ({loadError}). This is a live failure, not an empty
          directory — reload in a moment.
        </p>
      )}
...
function DirectoryCard({ entry }: { entry: DirectoryEntry }) {
  ...
  return (
    <article className="border-t border-navy pt-6 transition-colors duration-500 hover:bg-peach/20">
      <h2 className="font-serif text-4xl leading-tight text-navy">
        {entry.displayName ?? <span className="text-ocean">Agent #{entry.tokenId}</span>}
      </h2>
      {!entry.displayName && (
        <p className="mt-2 font-sans text-[10px] uppercase tracking-[0.25em] text-ocean">
          {entry.ensName ? "ENS identity did not resolve live" : "no ENS identity assigned"}
        </p>
      )}
      ...
      <div className="mt-6 flex flex-wrap items-center gap-x-6 gap-y-2">
        {entry.displayName && entry.ensAddress && (
          <a
