  ...
  try {
    ...
    if (marker && coach.ensName && roster.some((r) => r.ensName === marker.coach)) {
      ...
      const followUp: ChatMsg = result.ok
        ? {
            role: "user",
            content: `[consult result] ${result.coach.name} (${result.coach.ensName}) answered: «${result.reply}». Now answer your own athlete, weaving in and crediting your colleague's view. Do not use the <consult> marker again.`,
          }
        : {
            role: "user",
            content: `[consult result] Your colleague ${marker.coach} could not be reached (${result.error}). Answer your athlete on your own, mentioning that you tried to consult them. Do not use the <consult> marker again.`,
          };
